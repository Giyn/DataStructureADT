/**
 * @filename qgsort.c
 * @description qgsort source file
 * @author 许继元
 * @date 2020/4/24
 */
...
void MergeArray(int *a, int begin, int mid, int end, int *temp) {
    ...
    // 两两比较, 小的存入
    while (l_begin <= l_end && r_begin <= r_end) temp[k++] = a[l_begin] < a[r_begin] ? a[l_begin++] : a[r_begin++];
    // 若左半部分有剩余, 合并进去
    while (l_begin <= l_end) temp[k++] = a[l_begin++];
    // 若右半部分有剩余, 合并进去
    while (r_begin <= r_end) temp[k++] = a[r_begin++];

    for (int i = 0; i < end - begin + 1; i++) a[begin + i] = temp[i];
    free(temp);
}
...
int Partition(int *a, int begin, int end) {
    ...
    while (begin < end) {
        // 从右边开始找, 直到找到小于枢纽的
        while (begin < end && a[end] >= base) end--;
        Swap(a, begin, end);
        // 从左边开始找, 直到找到大于枢纽的
        while (begin < end && a[begin] <= base) begin++;
        Swap(a, begin, end);
    }
...
void CountSort(int *a, int size, int max) {
    ...
    // 收集
    for (int i = 0; i < size; i++) temp[a[i]] += 1;
    // 统计
    ...
    // 重新填入
    for (int i = 0; i < size; i++) a[i] = result[i];
    free(temp);
    free(result);
}

/**
 *  @name        : int getBit(int num, int pos)
 *  @description : 取出本次排序的位置上的数字
 *  @param       :
 */
...
void RadixCountSort(int *a, int size) {
    ...
    for (int pos = 1; pos <= len; pos++) {
        ...
        for (int i = 0, j = 0; i < BASE_SIZE; i++) {
            for (int k = 1; k <= radix[i][0]; k++) a[j++] = radix[i][k];
            radix[i][0] = 0; // 计数复位
        }
    }
}
...
void ColorSort(int *a, int size) {
    ...
    int i = 0;
    while (i <= right) {
        if (a[i] == 0) ColorSwap(a, i, left++);
        else if (a[i] == 2) ColorSwap(a, i, right--);
        i++;
    }
}
...
int Find_K(int *a, int begin, int end, int k) {
    ...
    if (index + 1 == k) return base;
    else {
        if (index + 1 < k) return Find_K(a, index + 1, end, k); // 说明第K大在它右边
        else return Find_K(a, begin, index - 1, k);
    }
}
...
int InputData(int *data) {
    ...
    for (int i = 0; i < MAX_SIZE && i < n; i++) fprintf(fpWrite, "%d ", data[i]);
    fclose(fpWrite);
...
int test(int *a, int *n) {
    ...
    do {
        *n = InputNumber();
        if (*n > MAX_SIZE || *n < 0) printf("Input number in [0-%d]:", MAX_SIZE);
        else break;
    } while (1);
    ...
    for (int i = 0; i < MAX_SIZE && i < *n; i++) fprintf(fpWrite, "%d ", a[i]);
    fclose(fpWrite);
