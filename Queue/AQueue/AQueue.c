/**
 * @filename AQueue.c
 * @description AQueue source file
 * @author 许继元
 * @date 2020/4/24
 */
...
void InitAQueue(AQueue *Q) {
    data_size = InputType();
    for (int i = 0; i < MAXQUEUE; i++) Q->data[i] = (void *) malloc(sizeof(data_size));
...
Status EnAQueue(AQueue *Q, void *data) {
    ...
    if (Q->rear == MAXQUEUE - 1) memcpy(Q->data[Q->rear = 0], data, data_size);
    else {
        if (Q->length == 0) memcpy(Q->data[Q->rear], data, data_size);
        else {
